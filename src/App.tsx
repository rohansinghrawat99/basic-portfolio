  ...
  return (
    <div className={`${ mode === 'day' ? 'bg-white text-black': 'bg-black text-white' } h-screen w-screen flex justify-center items-center`}>
      <div className='flex flex-col justify-around h-full w-[80%] min-w-[70%]'>
        <nav className='flex flex-col md:flex-row w-full justify-center items-center md:justify-between'>
          <img src={ `${mode === "night" ? '/icons/logo_white.svg' : '/icons/logo_black.svg'}`}  alt="" className='h-8 mb-10 md:h-14 cursor-pointer' />
        ...
        <div className='space-y-10'>
          ...
          <h1 className={`text-4xl md:text-6xl font-extrabold ${ mode === 'day' ? 'text-black': 'text-neon' } `}>Rohan Singh Rawat</h1>
        ...
        <footer className='flex w-full justify-center'>
          <div className='flex items-center font space-x-16'>
            ...
            <img src={ `${mode === "night" ? '/icons/twitter_icon_white.svg' : '/icons/twitter_icon_black.svg'}`} alt="ToggleOn" className='h-4 md:h-7 cursor-pointer' onClick={() =>  window.open('https://twitter.com/rohansrawat', '_blank', 'noreferrer noopener')}/>
          </div>
        </footer>
      </div>
    </div>
